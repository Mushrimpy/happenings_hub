...
def create_user(*, session: SessionDep, user_in: UserCreate) -> Any:
    ...
    Create new user.
    """
    user = auth_svc.get_user_by_email(session=session, email=user_in.email)
    if user:
        raise HTTPException(
            status_code=400,
...
def update_user_me(
    *, session: SessionDep, user_in: UserUpdateMe, current_user: CurrentUser
    ...
    if user_in.email:
        existing_user = auth_svc.get_user_by_email(session=session, email=user_in.email)
...
def delete_user_me(session: SessionDep, current_user: CurrentUser) -> Any:
    ...
    statement = delete(Activity).where(col(Activity.owner_id) == current_user.id)
...
def register_user(session: SessionDep, user_in: UserRegister) -> Any:
    ...
    user = auth_svc.create_user(session=session, user_create=user_create)
    return user
...
def update_user(
    *,
    ...
    db_user = auth_svc.update_user(session=session, db_user=db_user, user_in=user_in)
    return db_user
...
def delete_user(
    ...
    statement = delete(Activity).where(col(Activity.owner_id) == user_id)
